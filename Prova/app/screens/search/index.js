...
import { doc , collection, query, where, onSnapshot, documentId, startAt, endAt, orderBy } from 'firebase/firestore'
...
export function Search({ navigation, route }) {
    ...
    useEffect(()=>{
        ...
        const subscriver = onSnapshot(productsRef, {
            next : (snapshot) => {
              ...
              console.log(search)
              const arrayName = Object.values(products).map((item) => item.Title);
              const productsq2 = []
    ...
    useEffect(()=>{
      try{
        const listingsRef = collection(app_db, 'Listings')
    
        const q = query(
            listingsRef,
            where('Title', 'in', found)
        )
        setNotFound(false)
